...
<div class="content">
    <h1 class="title">{{ trans( 'crudlang::pages.create.model', [ 'name'=>trans_choice( '<% package.name %>::models.<% model.instances.single %>.name', true ) ] ) }}</h1>

    <form method="POST" action="{{ route( '<% package.name %>.<% model.instances.plural %>.store' ) }}">
